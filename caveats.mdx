...
- **We may occasionally add new fields to an existing endpoint without bumping the version.** Build your integration to ignore any fields it doesn’t recognize. We will never remove or rename a field without releasing a new API version—only additive changes occur within a version
